...
namespace SPAssistant.SPServices.Functions.Services
{
    class SPService
    {
        ...
        public SPService(string tenantId, string tenantUrl, string applicationId, X509Certificate2 x509Certificate2, TraceWriter log)
        {
            _tenant = tenantId;
        ...
        public async Task<string> ProcessCreateSiteRequest(CreateSiteRequest createSiteRequest)
        {
            var teamSiteUrl = await CreateSPSite(createSiteRequest);

            if (!string.IsNullOrWhiteSpace(teamSiteUrl) && !string.IsNullOrWhiteSpace(createSiteRequest.TemplateSiteUrl))
            {
                var customizationService = new SiteCustomisationService(_tenant,  _applicationId, _certificate509, _log);
                customizationService.Customize(createSiteRequest.TemplateSiteUrl, teamSiteUrl);
            }
        ...
        private async Task<string> CreateSPSite(CreateSiteRequest createSiteRequest)
        {
